...
def get_route_by_id(route_id: str):
    """
    Verifica si una ruta existe en la base de datos.

    :param route_id: ID de la ruta.
    :return: True si la ruta existe, False en caso contrario.
    """
    route = get_method("routes", {"route_id": route_id})
    return route is not None


def create_route_review(user_id: str, route_id: str, comment: str):
    """
    Crea o actualiza la reseña de un usuario para una ruta.
    Si el usuario ya tiene una reseña para esa ruta, se sobreescribe.

    :param user_id: ID token del usuario.
    :param route_id: ID de la ruta.
    :param comment: Texto de la reseña (máx. 500 caracteres).
    :return: Diccionario con los datos de la reseña creada/actualizada.
    """
    created_at = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    existing = get_method("route_reviews", {"user_id": user_id, "route_id": route_id})
    ...
    if existing:
        ...
        update_method(
            ...
            {"$set": {"comment": comment, "created_at": created_at}},
        )
    else:
        ...
        post_method("route_reviews", review_data)
...
def get_route_reviews(route_id: str):
    ...
    Incluye el nombre público y email del autor obtenidos de la colección users.

    :param route_id: ID de la ruta.
    :return: Lista de reseñas ordenada por created_at DESC.
    """
    reviews = get_method("route_reviews", {"route_id": route_id}, many=True)
    result = []
    for review in reviews:
        user = get_method("users", {"id_token": review.get("user_id")})
        user_name = None
        email = None
        if user:
