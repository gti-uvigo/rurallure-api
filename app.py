...
def create_poi():
    ...
    payload = {
        ...
        "languages": languages,
        "dev_mode": os.getenv("DB_NAME") != "rurallure"
    }
    headers = {
...
def function_rate_route():
    """
    Endpoint to publish a text review for a route.
    ---
    tags:
      - Routes
    parameters:
      - in: body
        ...
        schema:
          ...
          properties:
            ...
            comment:
              type: string
              example: "Great route, very scenic!"
            rating:
              type: integer
              example: 5
    responses:
      ...
      400:
        description: Datos de entrada inválidos
      401:
        description: Token de usuario no válido
      404:
        description: Ruta no encontrada
      500:
    ...
    if not route_id or not user_id or not comment:
        return jsonify({"error": "Missing required fields: route_id, user_id and comment are required"}), 400

    if len(comment) > 500:
        return jsonify({"error": "Comment exceeds maximum length of 500 characters"}), 400

    user = dto.get_user_by_id(user_id)
    if not user:
        return jsonify({"error": "Invalid or unrecognized user token"}), 401

    if not dto.get_route_by_id(route_id):
        return jsonify({"error": "Route not found"}), 404

    try:
        review = dto.create_route_review(user_id=user_id, route_id=route_id, comment=comment)
    except Exception as e:
        print("Error creating route review:", e)
        return jsonify({"error": "Internal server error"}), 500

    return jsonify({"data": review}), 200


@app.route('/get_route_reviews', methods=['POST'])
def function_get_route_reviews():
    """
    Endpoint to get all reviews for a specific route.
    ---
    tags:
      - Routes
    parameters:
    ...
    responses:
      200:
        description: Lista de reseñas de la ruta (ordenada por fecha descendente)
        schema:
          type: object
          properties:
            data:
    ...
    if not route_id:
        return jsonify({"error": "Missing required field: route_id"}), 400

    try:
        reviews = dto.get_route_reviews(route_id=route_id)
    except Exception as e:
        print("Error fetching route reviews:", e)
        return jsonify({"error": "Internal server error"}), 500
